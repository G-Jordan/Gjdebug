...
(function(){
  ...
  function updateDotVisibility(x){
    const w = progressArea.clientWidth; x = Math.max(0, Math.min(x, w)); dotIndicator.style.left = `${x}px`;
  }
  function setTimeAt(clientX){
    const rect = progressArea.getBoundingClientRect();
    const x = clientX - rect.left;
    const duration = document.querySelector('#main-audio').duration || 0;
    document.querySelector('#main-audio').currentTime = (x / rect.width) * duration;
