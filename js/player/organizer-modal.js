...
(function () {
  ...
  #org-modal .org-backdrop{position:absolute;inset:0;background:rgba(0,0,0,.55);backdrop-filter:blur(4px);}
  #org-modal .org-panel{position:relative;z-index:1;width:min(960px,92vw);max-height:82vh;overflow:hidden;border-radius:20px;
    border:1px solid #22303a;background:linear-gradient(135deg,rgba(123,92,255,.08),rgba(106,224,255,.06)),#0b0d10;
    box-shadow:0 18px 60px rgba(0,0,0,.55);}
  #org-modal header{display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid #22303a}
  #org-modal h3{margin:0;font:700 16px/1.2 system-ui,Segoe UI,Roboto,Arial;color:#e6eef5}
  #org-close{border:none;background:transparent;color:#e6eef5;opacity:.85;cursor:pointer;font-size:0;display:grid;place-items:center;border-radius:12px;padding:8px}
  #org-close:hover{background:rgba(255,255,255,.08)}
  #org-body{padding:12px 14px}

  /* Organizer core */
  .g73-organizer{color:#e6eef5}
  .org-controls{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px}
  .org-input,.org-select{background:#11161a;border:1px solid #1e2a33;color:#e6eef5;padding:10px 12px;border-radius:12px;min-width:220px;outline:none}
  .org-select{min-width:160px}
  .org-btn{cursor:pointer;border:1px solid #1e2a33;background:#11161a;color:#e6eef5;padding:10px 12px;border-radius:12px;font-weight:600}
  .org-btn.primary{background:linear-gradient(135deg,#7b5cff,#6ae0ff);color:#0b0d10;border-color:transparent}
  .org-file{cursor:pointer;border:1px dashed #1e2a33;padding:10px 12px;border-radius:12px;color:#9bb0c0}
  .org-list{display:grid;gap:8px;max-height:56vh;overflow:auto;padding-right:6px}

  .org-row{display:grid;grid-template-columns:28px 48px 1fr auto;align-items:center;gap:12px;background:#11161a;border:1px solid #1e2a33;border-radius:16px;padding:8px;transition:transform .12s ease}
  .org-row.dragging{opacity:.96;transform:scale(1.01)}
  .org-row.over{outline:2px dashed #6ae0ff;outline-offset:3px}
  .org-row[aria-grabbed="true"]{border-color:#7b5cff;box-shadow:0 0 0 3px rgba(107,142,255,.35)}

  /* DARKER DRAG HANDLE */
  .org-handle{
    cursor:grab; display:grid; place-items:center; border-radius:10px; padding:8px;
    color:#ffffff; background:rgba(0,0,0,.60); border:1px solid rgba(255,255,255,.10);
    touch-action:none; /* important for mobile pointer drag */
  }
  .org-handle:hover{ background:rgba(0,0,0,.72); }
  .org-handle:active{ cursor:grabbing; background:rgba(0,0,0,.80); }

  .org-cover{width:48px;height:48px;border-radius:12px;background:#0a0e12 center/cover no-repeat;border:1px solid #1e2a33}
  .org-name{font-weight:700;font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .org-artist{color:#9bb0c0;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .org-id{color:#9bb0c0;font-size:12px;padding:6px 10px;border:1px solid #1e2a33;border-radius:999px}
  .org-foot{display:flex;justify-content:space-between;align-items:center;color:#9bb0c0;font-size:12px;margin-top:10px}
  .org-list::-webkit-scrollbar{width:10px}
  .org-list::-webkit-scrollbar-thumb{background:linear-gradient(#7b5cff,#6ae0ff);border-radius:999px}
  .org-tip svg{vertical-align:middle}

  /* Small icon button inside .controls */
  .organize-btn{width:40px;height:40px;border-radius:14px;border:1px solid rgba(255,255,255,.12);background:transparent;color:#e8eef7;display:grid;place-items:center;cursor:pointer}
  .organize-btn:hover{background:rgba(255,255,255,.08)}
  .organize-btn .material-icons{font-size:22px;line-height:1}
  `;
  ...
  // ---------- HARDENED MOUNT + SAFE INITIAL RENDER ----------
  ...
  // Try a few times quickly, then observe DOM
  function ensureInjected(){
    let tries = 0;
    const maxTries = 20; // ~2s total with raf cadence
    function tick(){
  ...
  function tryApplySavedOrderOnBoot(){
    ...
    if (!saved.length) return; // nothing saved yet
    let ticks = 0;
    const maxTicks = 120; // ~2 seconds
  ...
  // ---------- Cover resolver (tries common folders/extensions) ----------
  ...
  function resolveCover(key){
    ...
    return `images/${key}.jpg`; // fast guess
  }
  ...
  // ---------- Modal show/hide ----------
  function openModal() {
    // Re-sync from current global, render, and push order globally so the main UI sees it
    syncFromGlobal();
    render(true /*fresh*/);
  ...
  function render(fresh = false) {
    ...
    // If working not ready, fall back to global
...
`;
  ...
  // Control wiring (deferred to when elements exist)
  document.addEventListener('click', (e)=>{
